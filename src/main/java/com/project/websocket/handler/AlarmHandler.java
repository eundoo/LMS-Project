package com.project.websocket.handler;
...
public class AlarmHandler extends TextWebSocketHandler {
	...
	public void sendMessage(String text) {
		...
		// 반복자를 사용해서 콜렉션에 저장된 모든 WebSecketSession객체의 sendMessage()를 실행해서
	...
	// 클라이언트와 서버가 연결이 완료되면 해당 클라이언트와 통신하는 WebSocketSession객체를 전달받을 수 있다.
	@Override
	...
	public void afterConnectionClosed(WebSocketSession session, CloseStatus status) throws Exception {
		// 위에서 정의한 콜렉션에서 WebSocketSession객체를 삭제한다.
		sessions.remove(session);
	}
	
	// 클라이언트와 서버가 통신 도중 오류가 발생하면 실행된다.
	@Override
